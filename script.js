...
url = [
  ...
  "https://m.zingmp3.vn/xhr/media/get-source?type=audio&key=ZmcGyzpzHmLWynTFmZHykhQABhdLiv",
  "https://m.zingmp3.vn/xhr/media/get-source?type=audio&key=ZHJmyzHLxLcJyHybnLmyZhWlVCdFum",
  "https://m.zingmp3.vn/xhr/media/get-source?type=audio&key=ZnJmtZHslxdRkhSyGybmkHTZXWzBhBzRz",
  "https://m.zingmp3.vn/xhr/media/get-source?type=audio&key=ZGcnydBLzDJStmyFHkHtkhQSdhdhEA",
  "https://m.zingmp3.vn/xhr/media/get-source?type=audio&key=LnJmtknadiHDJkCtGybmkmtkgWzdgVaJR",
];
url.map((item, index) => {
  return fetch(item)
...
function render(obj) {
  ...
  thumbnail.children[0].style = `background-image: url(${obj.photo})`;
  ...
  songs = $$(".song-list--item");
  for (song of songs) {
    if (song.dataset.id == obj.id) {
      song.classList.add("select");
      setTimeout(scollIntoview(), 200);
    }
  }

  rotate.cancel();
}
function scollIntoview() {
  song.scrollIntoView({
    behavior: "smooth",
    block: "end",
    inline: "center",
...
var rotate = thumbnail.children[0].animate([{ transform: "rotate(360deg)" }], {
  duration: 6000,
  iterations: Infinity,
});
rotate.pause();
...
audio.onpause = function () {
  // thumbnail.children[0].classList.remove("thumbnail");
  rotate.pause();
};
...
length.ontouchstart = function () {
  audio.ontimeupdate = function () {};
};

length.ontouchend = function () {
  audio.ontimeupdate = function () {
    update();
  };
};
...
document.onscroll = function () {
  scollPoin = window.scrollY;

  newWidth = width - scollPoin;
  if (newWidth < 0) {
    newWidth = 0;
  }
